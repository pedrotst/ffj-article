...
\begin{table}[!ht]
    \begin{tabularx}{.62\textwidth}{l|}
            ...
            \quad \hlnew{\texttt{refines~class C \{\={C} \={f}; KD \={M} \={MR}\}}} \\ \\
...
\texttt{A B C D} and \texttt{E} ranges over class names, \texttt{f} and \texttt{g} ranges over
field names; \texttt{m} ranges method name; \texttt{x} ranges over variable and \texttt{v} ranges over
values. We assume a that the set of variables includes the special variable \texttt{this}, which
...
An \ac{FFJ} program consists
of a set of class and refinement declaration as well as an expression. A class declaration
\texttt{CD} contains a list \texttt{\=C \=f} of fields, a constructor declaration \texttt{K},
and a list \texttt{\={M}} of method declarations. A class refinement \texttt{CR} contains
a list \texttt{\=C \=f} of fields, a constructor refinement \texttt{KR}, a list \texttt{\=M}
of method declaretions ane a list \texttt{\={MR}} of method refinements. The declaration of a 
class refinement is preceded by the keyword \texttt{refines}. Method declaration or a method 
